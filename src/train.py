...
def train(args=None):
    ...
    print(
        f"  Context length : {args.context_length} past + "
        f"{args.forward_context_length} future + 1 current = "
        f"{args.context_length + args.forward_context_length + 1} slots"
    )
    print(f"  Latent dim     : 128 (Encodec encoder output)")
    print(
        f"  Model          : Cross-attention transformer, "
        f"layers={args.latent_layers}"
    )
    ...
    datamodule.precompute_encodec_latents(
        encodec_model=_enc,
        encodec_sr=encodec_sample_rate,
        device=_device,
    ...
    print("Precomputation done — training will use cached latents.\n")
...
def parse_args():
    ...
    parser.add_argument("--forward_context_length", type=int, default=24)
    ...
    parser.add_argument("--wandb_project", type=str, default="liveify")
